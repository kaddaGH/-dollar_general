...
description = body.at('#collateral-tab-description').text rescue ''
short_description = body.at(".product-description-short").text.strip rescue ''
description = description.gsub(/Läs mer om produkten/,'')
description = description.gsub(/Ingredienser[^{}]+?\Z/,'').strip
...
if image_url.length>0
  image_url='http://162.243.94.227/imagesproxy/?url='+image_url
end
...
price ||= body.search(".special-price").at(".price").text.gsub("kr", "").unicode_normalize(:nfkc).strip rescue nil

price = price.gsub(',','.')
is_available = price.nil? ? "" : "1"
